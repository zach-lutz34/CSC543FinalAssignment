// The SHA256 hashing algorithm for the entire Book of Mark
// link to the book: https://quod.lib.umich.edu/cgi/r/rsv/rsv-idx?type=DIV1&byte=4697892

#include <string>

class sha256_hash_function
{
protected:
...
public:
    ...
    void final_dig(unsigned char *digest);
    static const unsigned int PAD_SIZE = (256 / 8);
...
const unsigned int sha256_hash_function::hash_keys[64] =
...
//compresses hash
void sha256_hash_function::compress(const unsigned char *message, unsigned int block_new)
{
    reg_32 w[64];
    reg_32 buffer[8];
    reg_32 t1, t2;
    const unsigned char *block_s;
    int a;
    int b;
...
//creates final value
void sha256_hash_function::final_dig(unsigned char *digest)
{
    unsigned int block_new;
...
std::string sha256(std::string input)
{
    ...
    memset(digest,0,sha256_hash_function::PAD_SIZE);

    sha256_hash_function obj = sha256_hash_function();
    obj.s_reg();
    obj.adj_digest( (unsigned char*)input.c_str(), input.length());
    obj.final_dig(digest);

    char buf[2*sha256_hash_function::PAD_SIZE+1];
    buf[2*sha256_hash_function::PAD_SIZE] = 0;
    for (int s = 0; s < sha256_hash_function::PAD_SIZE; s++)
        sprintf(buf+s*2, "%02x", digest[s]);
...
int main(int hash, char *arg[])
{

    string outputBookofMark = "Mark.1\n"
...
"";
    string outputText = sha256(outputBookofMark);

    cout << "This is the SHA-256 hash algorithm for the entire Book of Mark: ” << outputText << endl;
    return 0;
}
